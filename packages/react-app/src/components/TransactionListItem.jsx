...
const TransactionListItem = function ({item, mainnetProvider, blockExplorer, price, readContracts, contractName, children}) {
  ...
  console.log("🔥🔥🔥🔥", item)
  let txnData;
  let txnDataFragmentName;
  let txnDataArgument;
  try {
    if(item.data !=0)
    {
      txnData = readContracts[contractName].interface.parseTransaction(item);
      txnDataFragmentName = txnData.functionFragment.name;
      txnDataArgument = txnData.args[0];
    }
    else {
      txnDataFragmentName = "send";
      txnDataArgument = item.to;
    }
  } catch (error){
  ...
  return <>
    <TransactionDetailsModal
      ...
      price={price}
    />
  {(txnData || txnDataFragmentName =="send" )&& <List.Item key={item.hash} style={{ position: "relative" }}>
      <div
        style={{
          position: "absolute",
          top: 55,
