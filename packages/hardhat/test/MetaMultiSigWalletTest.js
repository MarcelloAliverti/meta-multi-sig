...
describe("MetaMultiSigWallet Test", () => {
  ...
  describe("Testing MetaMultiSigWallet functionality", () => {
    ...
    it("Update Signatures Required to 2", async () => {
      let newSigner = addr1.address;
      ...
      nonce = await metaMultiSigWallet.nonce();

      let callData1 = metaMultiSigWallet.interface.encodeFunctionData("updateSignaturesRequired",[2]);

      let hash1 = await metaMultiSigWallet.getTransactionHash(nonce, to, value, callData1);

      const signature1 = await owner.provider.send("personal_sign", [hash1, owner.address]);
      // Double checking if owner address is recovered properly, executeTransaction would fail anyways
      expect(await metaMultiSigWallet.recover(hash1, signature1)).to.equal(owner.address);

      try{
        const txres = await metaMultiSigWallet.executeTransaction(metaMultiSigWallet.address, value, callData1, [signature1]);
        const result_txt =  await txres.wait();
      }catch(error)
      {
        console.log(error);
      }
    ...
    it("Remove a signer", async () => {
      ...
      let callData = metaMultiSigWallet.interface.encodeFunctionData("addSigner",[newSigner, 2]);
      ...
      const txres = await metaMultiSigWallet.executeTransaction(metaMultiSigWallet.address, value, callData, [signature]);

      const result_txt =  await txres.wait();

      expect(await metaMultiSigWallet.isOwner(newSigner)).to.equal(true);

      let oldSigner = newSigner;

      let nonce1 = await metaMultiSigWallet.nonce();

      value = 0;

      let callData1 = metaMultiSigWallet.interface.encodeFunctionData("removeSigner",[oldSigner, 1]);

      let hash1 = await metaMultiSigWallet.getTransactionHash(nonce1, to, value, callData1);

      const signature1 = await addr1.provider.send("personal_sign", [hash1, addr1.address]);

      const signature2 = await owner.provider.send("personal_sign", [hash1, owner.address]);

      const tx1res = await metaMultiSigWallet.executeTransaction(metaMultiSigWallet.address, value, callData1, [signature1,signature2]);

      const result1_txt =  await tx1res.wait();

      expect(await metaMultiSigWallet.isOwner(oldSigner)).to.equal(false);
